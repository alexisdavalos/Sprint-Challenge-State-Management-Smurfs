...
- [ ] What problem does the context API help solve?
  ...
  Context API tries to solve the problem of prop drilling

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  Actions are basically functions that interface with the UI and then proceed to send an "action type" and potentially an "action payload" to the reducer.

  Reducers take the action and action payload and update the state tree that is held in the redux store.

  Store is the global state that wraps around the app and provides context/state to connected components

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is the Redux Store, its a global state tree that is available to any connected components. Component state is state that initialized within a component, but only available inside of that component and its children through prop drilling. A good time to use global state is whenever you have planned out the states of your Finite State Application and realize you will need data in multiple component trees. A good time to user component state is when you are only using that data inside of that component and or its children.  

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  Redux Thunk allows us to perform asynchronous functions in our actions. This means that instead of immediately sending the action over to the reducer, Redux Thunk aka middleware allows us to dispatch specific actions/paylods to the reducer. 

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

  I really like Redux. It is a lot of boilerplate to get it working, but the beauty of being able to connect to global state opens up a whole new world of possibilities and I am really looking forward to building more and more with this new framework. 
